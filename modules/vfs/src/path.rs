...
impl Path {
    ...
    pub fn parent(&self) -> Option<&Path> {
        ...
        comp.and_then(move |p| match p {
            Component::Normal(_) | Component::CurrentDir | Component::ParentDir => {
                Some(comps.as_path())
            }
            _ => None,
        })
    ...
    /// This will resolve `.` and `..` components, return None if the path is invalid.
    /// (e.g., `a/b/../c` will return `a/c`, but `/../b` will return None).
...
impl PathBuf {
    ...
    fn _push(&mut self, path: &Path) {
        ...
        if path.is_absolute() {
            self.inner.clear();
        } else if !self.inner.ends_with('/') && !self.inner.is_empty() {
            self.inner.push('/');
        }
